#!/usr/bin/env python3
r"""
251102 곤포사일리지 타일 기반 추론 파이프라인 v12
=================================================

v12 정확도 개선 - 품질 필터링 추가 (v11 개선):
① v11 기능 유지:
   - make_tiles 경계 보강 (40% overlap, 경계 50%)
   - 2차 추론 (경계 타일만)
   - 이원화 병합
   - 과대 마스크 재분할
   - area 기반 보정 카운팅

② v12 신규 - 품질 필터링 (오검출 제거):
   - **최종 필터링**: 이원화 병합 후 품질 검증
   - **신뢰도 필터**: confidence ≥ 0.4 (낮은 신뢰도 제거)
   - **면적 필터**: area ≥ 3000px (너무 작은 검출 제거)
   - **비율 필터**: aspect_ratio ≤ 5.0 (너무 길쭉한 검출 제거)

③ conf_threshold 조정 (정확도 향상):
   - 1차 추론: conf=0.25 (v11: 0.18 → v12: 0.25)
   - 2차 추론: conf=0.22 (v11: 0.15 → v12: 0.22)

v12의 핵심 개선 (v11 대비):
- **오검출 감소**: 3단계 품질 필터링 (신뢰도 + 면적 + 비율)
- **정확도 향상**: conf_threshold 상향 조정
- **정밀도 증가**: 객체가 없는 부분 검출 최소화

작업:
1. SHP 기반 TIF 크롭 (2.0m 버퍼)
2. 타일 생성 (40% overlap, 경계 50%)
3. 2차 추론 (conf=0.25 / 0.22, scale=1.5 / 1.7)
4. Global WBF
5. 과대 마스크 재분할 (area > 120000)
6. 이원화 병합
7. **품질 필터링 (신뢰도 ≥0.4, 면적 ≥3000px, 비율 ≤5.0)** ← v12 신규
8. area 기반 카운팅
...
출력:
- inference_system/output_251102_v12/

작성일: 2025-11-02 (v12 - Quality Filtering for Precision)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('251102_v12_pipeline.log', encoding='utf-8')
    ]
)
...
class GonpoInferencePipeline:
    ...
    def __init__(
        ...
        conf_threshold: float = 0.25,  # v12: 0.18 → 0.25 (정확도 향상)
        nms_iou_threshold: float = 0.60,
        inference_scale: float = 1.5,
        file_prefix: str = "251102_v12"
    ):
        """
        Args:
        ...
        logger.info("=" * 80)
        logger.info("251102 곤포사일리지 추론 파이프라인 v12 시작")
        logger.info("=" * 80)
    ...
    def run(self):
        ...
        try:
            ...
            # Step 2: 타일 기반 추론
            logger.info("\n" + "=" * 80)
            logger.info("Step 2: 타일 기반 추론 (v12: 품질 필터링 추가)")
            logger.info("=" * 80)
...
def main():
    ...
    v12 특징 (v11 개선 - 정확도 향상):
    ① 품질 필터링: 신뢰도 ≥0.4, 면적 ≥3000px, 비율 ≤5.0
    ② conf_threshold 상향: 0.25 (1차), 0.22 (2차)
    ③ 오검출 최소화: 3단계 필터링으로 정밀도 향상
    """
    # 경로 설정
    MODEL_PATH = r"C:\Users\LX\dbwjdakrso4235-sys\runs\segment\silage_optimized\weights\best.pt"
    TIF_PATH = r"E:\namwon_ai\input_tif\금지면_1차.tif"
    SHP_PATH = r"E:\namwon_ai\gonpo\gonpo_251028_fixed.shp"
    OUTPUT_DIR = "inference_system/output_251102_v12"
    ...
    pipeline = GonpoInferencePipeline(
        ...
        tile_size=1024,
        overlap_ratio=0.40,  # v12: 40% (경계 50%)
        conf_threshold=0.25,  # v12: 정확도 향상 (v11: 0.18 → v12: 0.25)
        nms_iou_threshold=0.60,
        inference_scale=1.5,
        file_prefix="251102_v12"
    )
    ...
    # 최종 요약 출력
    print("\n" + "=" * 80)
    print("v12 처리 완료! (정확도 개선 - 품질 필터링 추가)")
    print("=" * 80)
    ...
    print("=" * 80)
    print("\nv12 개선사항 (오검출 제거):")
    print("  1. conf_threshold 상향:")
    print("     - 1차 추론: 0.25 (v11: 0.18)")
    print("     - 2차 추론: 0.22 (v11: 0.15)")
    print("  2. 품질 필터링 (3단계):")
    print("     - 신뢰도 필터: confidence ≥ 0.4")
    print("     - 면적 필터: area ≥ 3000px")
    print("     - 비율 필터: aspect_ratio ≤ 5.0")
    print("  3. 효과: 오검출 최소화, 정밀도 향상")
    print("=" * 80)
